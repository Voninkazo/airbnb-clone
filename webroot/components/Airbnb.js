...
export default function Airbnb() {
  ...
  return (
    <div className='big-container'>
      ...
      <ButtonsComponent handleClick={handleClick} />
      ...
      <div className='stays'>
        <div className='heading_container'>
          ...
          {filteredByGuests.length != 0 ? (
            <p>
              {filteredByGuests.length}{' '}
              {filteredByGuests.length > 1 ? 'stays' : 'stay'}
            </p>
          ) : (
        ...
        {filteredByGuests.map((stay) => {
          return <Stays key={stay.id} stay={stay} allStays={allStays} />
        })}
      </div>
    </div>
  )
